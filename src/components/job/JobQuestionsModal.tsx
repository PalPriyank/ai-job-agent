...
const JobQuestionsModal: React.FC<Props> = ({ isOpen, onClose, questions: initialQuestions, selectedJob ,fetchjob}) => {
    const [questions, setQuestions] = useState(initialQuestions);
    const [editing, setEditing] = useState<{ qIndex: number; oIndex: number } | null>(null);
    ...
    const handlePublishJob = async () => {
        try {
            setLoader(true);
            onClose();
            const publishedJob = await publishJob(selectedJob?.title, selectedJob.skills, selectedJob.recommended_location)
            fetchjob();
        } catch (error) {
            ...
            {isOpen && (
                ...
                >
                    ...
                    >
                        ...
                        {/* Header */}
                        <h2 className="text-xl font-bold mb-1 text-gray-800 text-center">Screening Questions</h2>
                        <p className="text-sm text-gray-500 mb-6 text-center">
                            Please review, edit, and approve these questions to qualify ideal candidates.
                        </p>
                        ...
                        {questions.map((q, qIndex) => (
                            <div key={qIndex} className="mb-6 border rounded-xl p-4">
                                ...
                                {q.options.map((opt, oIndex) => {
                                    ...
                                    const isEditing = editing?.qIndex === qIndex && editing?.oIndex === oIndex;
                        ...
                        <div className="flex justify-end mt-4">
                            <button onClick={handlePublishJob} className="bg-purple-600 text-white px-4 py-2 w-full rounded-lg hover:bg-purple-700">
                                Approve Screening Questions
                            </button>
                        </div>
                    </motion.div>
                </motion.div>
